...
class UsuarioDao{
    public function estaRegistradoUsuario(UsuarioBean $usuobj){
        ...
        $sql = "SELECT * FROM login WHERE BINARY correo='" .$usuobj->getCorreo()."' AND contraseña='" . $usuobj->getContraseña()."'";       //jala los valores de usuarioBean
    ...
    public function devolverTipoUsuario(UsuarioBean $usuobj){
        $objc = new ConexionBD();
        $con=$objc->getConexionBD();
        $sql = "SELECT * FROM login WHERE BINARY correo='" .$usuobj->getCorreo()."' AND contraseña='" . $usuobj->getContraseña()."'";
    }
}
